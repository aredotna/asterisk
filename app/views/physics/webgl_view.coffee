...
module.exports = class WebGLRenderer extends PhysicsRenderer
	offset:
		x: 0
		y: 0
	zoom: 1
	...
	addParticle: (particle) ->
		...
		@colours.push rgba[0], rgba[1], rgba[2], 1
	...
	render: (physics) ->
		...
		if @renderParticles
			...
			for p in physics.particles
				vertices.push (p.pos.x + @offset.x) * @zoom, (p.pos.y + @offset.y) * @zoom, 0.0
		...
		if @renderSprings and physics.springs.length > 0
			...
			for s in physics.springs
				vertices.push (s.p1.pos.x + @offset.x) * @zoom, (s.p1.pos.y + @offset.y) * @zoom, 0.0
				vertices.push (s.p2.pos.x + @offset.x) * @zoom , (s.p2.pos.y + @offset.y) * @zoom, 0.0
